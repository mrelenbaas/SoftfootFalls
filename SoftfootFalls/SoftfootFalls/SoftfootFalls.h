...
void WindowsProcess(char* text)
{
	...
	size_t requiredSize;
	mbstowcs_s(&requiredSize, nullptr, 0, text, _TRUNCATE);
	std::vector<wchar_t> wcBuffer(requiredSize);
	errno_t err = mbstowcs_s(&requiredSize, wcBuffer.data(), wcBuffer.size(), text, _TRUNCATE);
	if (err == 0) {
	...
	std::cout << "Process ID: " << processInformation.dwProcessId << "\n";
	std::cout << "Thread ID: " << processInformation.dwThreadId << "\n";
